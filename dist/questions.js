...
export function dietQuestions(current, injectionGuard) {
    const questions = {
        [Q_NEEDS_CONTENTS]: {
            type: 'noul',
        ...
        [Q_REPLACEABLE]: {
            type: 'noul',
            instructions: 'The same information as the current tool result would come back if the call were run again, or that information is already present elsewhere in the session.',
            criteria: criteria('Running the call again returns the same information, or the session already holds it.', 'Running the call again would return different information: a fresh random value, a new timestamp, a changed file, or a one-off computation.'),
        },
        [Q_KEEP_CALL]: {
            type: 'noul',
            ...
            criteria: criteria('Knowing that this call was made with these arguments matters ahead.', 'Whether this call happened does not matter ahead.'),
        },
    };
    if (injectionGuard) {
        questions[Q_AGENT_DIRECTED] = {
            ...
            criteria: criteria('The text gives instructions, directives or messages to an assistant.', 'The text is output, data, code, logs, stack traces or error messages for a reader.'),
        };
        questions[Q_BEHAVIOUR_CHANGE] = {
            ...
            criteria: criteria('The text directs the assistant to take or avoid an action.', 'The text only describes a result and directs nothing.'),
        };
    }
    return questions;
}
