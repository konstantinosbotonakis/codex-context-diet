...
export function readCache(env, sessionId, config) {
    try {
        const entries = [];
...
export function appendCache(env, sessionId, entry, config) {
    try {
        mkdirSync(sessionsDir(env), { recursive: true });
    }
    catch {
    ...
    try {
        if (statSync(path).size <= config.cacheMaxBytes)
            return;
        const kept = [];
        let bytes = 0;
...
export function countSessions(env) {
    try {
