...
export function resurrectionPath(env, sessionId) {
...
export function buildSnapshot(env, sessionId, config) {
    const lines = [RESURRECTION_HEADER];
    const { goal } = readGoal(env, sessionId);
    if (goal.trim().length > 0)
        lines.push('Goal: ' + clip(goal.replace(/\n/g, ' / '), 300));
    const files = [...new Set(readTouches(env, sessionId).flatMap((touch) => touch.paths))]
        .filter((path) => path !== '*')
        .slice(-8);
    if (files.length > 0)
        lines.push('Files written: ' + files.join(', '));
...
export async function handleCompaction(payload, env) {
    try {
        const config = loadConfig(env);
        if (!config.compactionResurrection)
            return '';
...
export function takeResurrection(env, sessionId, config) {
    if (!config.compactionResurrection)
        return null;
