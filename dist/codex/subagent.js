 ...
 */
export const SUBAGENT_CONTRACT = 'Return concise findings and evidence: the conclusion, the relevant files, the important evidence, ' +
...
export const SUBAGENT_CONTEXT = 'A subagent has finished and its latest message is about to be returned to the parent agent. ' +
...
export function decideSubagentVerdict(answers, config) {
    ...
    if (redundant >= threshold && actionable >= threshold) {
        return {
            action: 'revise',
            reason: 'Condense the response to the conclusion, the relevant files, the evidence and the test ' +
    ...
    if (satisfied < threshold || actionable < threshold || evidence < threshold) {
        return {
            action: 'revise',
            reason: 'Finish the request and return a concise, evidence-backed result: conclusion, relevant files, ' +
...
function recordIntervention(env, agentId) {
    try {
...
export async function handleSubagent(payload, env) {
    try {
