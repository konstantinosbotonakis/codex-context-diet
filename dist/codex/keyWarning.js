...
const MESSAGES = {
    missing: 'Context Diet: no TypeSafe API key found, so Jev was not used and tool results were kept in full. ' +
        'Set TYPESAFE_API_KEY or write the key to ~/.typesafe_key.',
    rejected: 'Context Diet: TypeSafe rejected the API key, so Jev was not used and tool results were kept in full. ' +
        'Check the key, then update ~/.typesafe_key or TYPESAFE_API_KEY.',
};
export function keyWarningMessage(problem) {
    return MESSAGES[problem];
}
/** Pulls the HTTP status out of a transport error, so a rejected key can be named. */
export function problemFromError(message) {
 ...
 */
export function keyWarning(env, sessionId, problem, now = new Date()) {
    ...
    try {
        ...
        if (Number.isFinite(at) && now.getTime() - at < 60 * 60 * 1000)
            return null;
    }
    catch {
        // no record yet
...
//# sourceMappingURL=keyWarning.js.map