...
async function providerCommand() {
    ...
    if (action === 'set') {
        ...
        if (target === 'laya')
            process.stdout.write('run `context-diet provider warm` once to load the checkpoint\n');
        return 0;
    }
    if (action === 'warm') {
        const started = Date.now();
        try {
            const reply = await warmLaya(config, env);
            process.stdout.write('laya: loaded ' + (reply.model ?? config.layaModel) + ' on ' + (reply.device ?? 'auto') +
...
if (command === 'status')
    ...
    process.exitCode = await evalCommand();
else if (command === 'benchmark')
    process.exitCode = await benchmark();
else if (command === 'provider')
    process.exitCode = await providerCommand();
