...
function classify(line) {
    ...
    for (const pattern of PATTERNS) {
        if (!pattern.pattern.test(line.text))
            continue;
        if (best === null || pattern.priority < best.priority)
            best = pattern;
    }
    return best;
}
export function sampleResult(text, options) {
    ...
    if (text.length <= budget)
        return { text, signalLines: 0, omitted: 0 };
    ...
    for (const line of lines) {
        if (line.start < headEnd || line.end > tailStart)
            continue;
    ...
    const assemble = (keep) => {
        ...
        if (cursor < tailStart)
            out += '[... ' + (tailStart - cursor) + ' chars omitted ...]\n';
