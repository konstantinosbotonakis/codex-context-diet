...
describe('decisions', () => {
  ...
  it('keeps, drops the result, or drops the call based on the keep probabilities', () => {
    expect(decideCall(unpinned, { keepCall: 0.9, keepResult: 0.7 }, 0.5).action).toBe('keep');
    expect(decideCall(unpinned, { keepCall: 0.9, keepResult: 0.2 }, 0.5).action).toBe('drop_result');
    expect(decideCall(unpinned, { keepCall: 0.1, keepResult: 0.2 }, 0.5).action).toBe('drop_call');
    expect(decideCall({ ...unpinned, pinned: true }, { keepCall: 0, keepResult: 0 }, 0.5)).toMatchObject({
      action: 'keep',
      reason: 'pinned',
    });
  });
  ...
  it('removes dropped calls and truncates dropped results', () => {
    ...
    const decisions = [
      decideCall(calls[0]!, { keepCall: 0.1, keepResult: 0.1 }, 0.5),
      decideCall(calls[1]!, { keepCall: 0.9, keepResult: 0.1 }, 0.5),
      decideCall(calls[2]!, { keepCall: 0.9, keepResult: 0.9 }, 0.5),
    ];
    ...
    expect(kept[2]?.toolUses[0]?.text).toMatch(
      new RegExp(`^${'x'.repeat(300)}\\n\\[codex-context-diet truncated 1700 chars`),
    );
  ...
  it('honours truncateHeadChars, including a zero head', () => {
    const messages = transcript();
    const calls = collectToolCalls(messages, 0);
    const decisions = [decideCall(calls[0]!, { keepCall: 0.9, keepResult: 0.1 }, 0.5)];
    ...
    expect(kept[2]?.toolResults?.[0]?.text).toBe(
      `${original.slice(0, 50)}\n[codex-context-diet truncated ${total - 50} chars of this tool result; re-run the tool if needed]`,
    );
    ...
    expect(noHead[2]?.toolResults?.[0]?.text).toBe(
      `[codex-context-diet truncated ${total} chars of this tool result; re-run the tool if needed]`,
    );
  });
});
...
describe('truncated result note', () => {
  ...
  it('keeps the head, states the omitted count, and points at a re-run', () => {
    expect(truncatedResultText(long, false, 4)).toBe(
      'xxxx' + nl + '[codex-context-diet truncated 196 chars of this tool result; re-run the tool if needed]',
    );
    expect(truncatedResultText(long, true, 4)).toContain('(error)');
