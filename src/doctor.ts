...
function readJson(path: string): { value: Record<string, unknown> | null; error: string | null } {
  try {
...
function mcpHandshake(env: NodeJS.ProcessEnv, serverPath: string): string | null {
...
export function runDoctor(env: NodeJS.ProcessEnv): Check[] {
  const checks: Check[] = [];
  ...
  try {
    for (const name of readdirSync(sessions)) {
      if (!statSync(join(sessions, name)).isDirectory()) continue;
      const file = cachePath(env, name);
      if (!existsSync(file)) continue;
      cacheFiles += 1;
      if (readJson(file).error !== null) corrupt += 1;
    }
  } catch {
  ...
  const wired = hooks.value?.hooks && typeof hooks.value.hooks === 'object'
    ? Object.keys(hooks.value.hooks as Record<string, unknown>)
    : [];
  checks.push({
    name: 'hooks',
  ...
  const referenced = new Set<string>(['dist/cli.js', 'dist/mcp-server.js']);
  const walk = (value: unknown): void => {
    if (typeof value === 'string') {
      for (const match of value.matchAll(/dist\/[A-Za-z0-9/._-]+\.js/g)) referenced.add(match[0]);
      return;
    }
