...
export function sampleTranscript(): Message[] {
  return [
...
export async function verifyCompaction(deps: { asker: JevAsker }): Promise<VerificationReport> {
  const checks: VerificationCheck[] = [];
  const record = (name: string, ok: boolean, detail: string): boolean => {
    checks.push({ name, ok, detail });
    return ok;
  };
  const attempt = (name: string, run: () => string): boolean => {
    try {
  ...
  attempt('token-estimator', () => {
    if (estimateTokens('') !== 0) throw new Error('the empty string is not free');
    if (estimateTokens('hello world') !== 2) throw new Error('word cost changed');
    if (estimateTokens('internationalization') !== 4) throw new Error('long-word cost changed');
    if (estimateTokens('12345678') !== 4) throw new Error('digit cost changed');
    return 'four calibration cases hold';
  });

  attempt('collect-tool-calls', () => {
    const ids = calls.map((call) => call.id).join(',');
    if (ids !== 't1,t2,t3') throw new Error('ids are ' + ids);
    if (calls[1]?.isError !== true) throw new Error('the failing call is not marked as an error');
    if (calls[1]?.resultChars !== sampleLog().length) throw new Error('resultChars is wrong');
    return 'three calls paired with their results';
  });

  let stateTokens = 0;
  let state: object | string = {};
  attempt('fit-state', () => {
    ...
    if (json.includes(LOG_LINE.trim())) throw new Error('the log body leaked into the state');
    if (!json.includes('Fix the failing test')) throw new Error('the goal is missing from the state');
  ...
  attempt('batch-calls', () => {
    ...
    const batched = batchCalls(calls, stateTokens, MAX_REQUEST_TOKENS)
      ...
      .map((call) => call.id)
      .join(',');
    if (batched !== order) throw new Error('the order changed: ' + batched);
    return 'all questions fit one request';
  });

  let answers: DietAnswers | null = null;
  try {
    const response = await deps.asker.ask(
      state,
      dietQuestions({ tool: 'Bash', inputLine: 'npm test', resultChars: sampleLog().length }, true),
    );
    answers = {
  ...
  try {
    if (answers === null) throw new Error('no answers to decide with');
  ...
  attempt('apply-decisions', () => {
    ...
    if (json.includes(LOG_LINE.trim())) throw new Error('the dropped result survived');
    if (!json.includes('[codex-context-diet truncated')) throw new Error('the truncated result has no note');
    if (kept[0] !== messages[0]) throw new Error('an untouched message was rebuilt');
  ...
  attempt('reduction', () => {
    ...
    if (!(charsAfter < charsBefore)) throw new Error('the result did not shrink');
    if (!(reduction > 0)) throw new Error('the reduction ratio is not positive');
