...
const TOOLS = [
  ...
  tool(
    'quality_guard',
    'Optional completion-quality check for the Stop event. Off unless qualityGuard is enabled; returns a continuation reason when the turn should not finish yet.',
    {
      turn_id: { type: 'string' },
      last_assistant_message: { type: 'string' },
      stop_hook_active: { type: 'boolean' },
  ...
  tool(
    'jev_boolean',
    'Ask the configured System One provider a yes/no question about a state and return the probability. Cheaper than a reasoning model for one calibrated judgement. Answers from Jev by default, or from a local checkpoint when one is configured.',
    { state: {}, question: { type: 'string' } },
    ['state', 'question'],
  ),
  tool(
    'jev_choice',
    'Ask the configured System One provider to pick one option for a state and return the choice with its probability distribution. Include a no-match option when nothing may fit.',
    { state: {}, question: { type: 'string' }, options: { type: 'array', items: { type: 'string' } } },
    ['state', 'question', 'options'],
  ),
  tool(
    'jev_score',
    'Ask the configured System One provider to rate a state along an ordered list of levels and return the probability-weighted score. Provide the levels from lowest to highest, each one able to stand alone.',
    { state: {}, question: { type: 'string' }, levels: { type: 'array', items: { type: 'string' } } },
    ['state', 'question', 'levels'],
  ),
  tool(
    'jev_ask',
    'Ask the configured System One provider several independent questions about one state in a single request. The questions run in parallel and cannot see one another, so state every speculative premise explicitly. Use it for classification, filtering, routing and other simple judgements over the same text.',
    {
      state: {},
      questions: {
        type: 'array',
...
async function callTool(name: string, args: Args, env: NodeJS.ProcessEnv): Promise<unknown> {
  ...
  if (name === 'jev_ask') {
    ...
    if (entries.length === 0) return fail('questions must be a non-empty array');
    if (entries.length > 8) return fail('at most 8 questions are accepted in one request');
    const questions: JevQuestions = {};
    const kindById: Record<string, 'noul' | 'choice' | 'score'> = {};
    const itemsById: Record<string, string[]> = {};
    for (const entry of entries) {
      if (!entry || typeof entry !== 'object' || Array.isArray(entry)) return fail('every question must be an object');
      const record = entry as Record<string, unknown>;
      const id = text(record.id).trim();
      const kind = text(record.type).trim();
      const prompt = text(record.question).trim();
      if (id.length === 0) return fail('every question needs a non-empty id');
      if (Object.prototype.hasOwnProperty.call(questions, id)) return fail('duplicate question id: ' + id);
      if (prompt.trim().length === 0) return fail('question ' + id + ' has no text');
      ...
      if (values.length < 2) return fail('question ' + id + ' needs at least two ' + noun);
      if (values.length > 20) return fail('question ' + id + ' has more than 20 ' + noun);
      itemsById[id] = values;
    ...
    for (const id of Object.keys(questions)) {
      const answer = (response.answers[id] ?? {}) as unknown as Record<string, unknown>;
      ...
      const score =
        typeof answer.score === 'number' && Number.isFinite(answer.score) && answer.score >= 0 && answer.score <= 1
          ? answer.score
          : null;
      if (score === null) return fail('question ' + id + ' returned a score outside 0..1');
