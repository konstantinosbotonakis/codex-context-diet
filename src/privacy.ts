...
interface Rule {
  ...
  placeholder: string;
  pattern: RegExp;
  /** Replacement string, which may reuse capture groups. Defaults to the placeholder. */
  replacement?: string;
}
...
export function redactValue(value: unknown, mode: PrivacyMode): unknown {
  ...
  if (value !== null && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [key, redactValue(item, mode)]),
    );
  }
  return value;
}
 ...
 */
export function isNeverSendInput(toolName: string, inputLine: string, config: DietConfig): boolean {
  if (config.privacyMode === 'off') return false;
  if (config.neverSendTools.some((tool) => tool.toLowerCase() === toolName.toLowerCase())) return true;
  if (config.privacyMode !== 'strict' || config.neverSendPaths.length === 0) return false;
  ...
  return candidates.some((candidate) =>
    patterns.some((pattern) => pattern.test(candidate) || pattern.test(basename(candidate))),
  );
}
