...
const output = (iteration: number): string => 'run ' + iteration + '\n' + bulk.repeat(1600);
const payload = (iteration: number): string =>
  JSON.stringify({
...
function mcpClient(env: NodeJS.ProcessEnv): McpChild {
...
export async function runBenchmark(options: BenchOptions = {}): Promise<BenchReport> {
  ...
  for (let iteration = 0; iteration < iterations; iteration += 1) {
    ...
    await client.call('tools/call', {
      name: 'post_tool_use',
      arguments: JSON.parse(payload(iteration)) as Record<string, unknown>,
    });
  ...
  if (options.live) {
    ...
    if (key === null) {
      ...
      await new JevClient({ apiKey: key, model: config.model }).ask(
        { ping: 'ok' },
...
export function renderBenchmark(report: BenchReport): string {
  const format = (value: number): string => value.toFixed(1).padStart(7);
  const row = (label: string, stat: Stat): string =>
    label.padEnd(28) + format(stat.p50) + format(stat.p95) + format(stat.mean);
  const startupRow = (label: string, value: number): string =>
    label.padEnd(28) + format(value) + '      -        -   once per session';
  const lines = [
