...
export function dietQuestions(current: QuestionInput, injectionGuard: boolean): JevQuestions {
  const questions: JevQuestions = {
    [Q_NEEDS_CONTENTS]: {
      ...
      criteria: criteria(
        'The next steps depend on these exact contents. That includes a failure, an error, a stack trace, a diff, a migration, a file the work is about to change, a value the next command needs, or anything not already captured elsewhere in the session.',
        'The next steps do not depend on these contents. The run was clean, the output repeats by re-running, or the session already holds the same information.',
      ),
    },
    [Q_REPLACEABLE]: {
      type: 'noul',
